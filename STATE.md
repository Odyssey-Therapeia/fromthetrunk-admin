...
- **Next concrete action**: P2-00/01/02/04/10/11 DONE (P2-04 7307d4e — repaired: calculateOrderTotals now the single charge source, both routes wired, flag-OFF mutation-proven byte-identical; P2-04a client-server-totals deferred). Next: **P2-05 inventory v2 schema + dual-write** — quantity_available + reservations table, migration (IF NOT EXISTS — drift trap), compat layer keeps stockStatus derivable, atomic claim via reservations insert, release-reservations cron rewrite; build files+tests only, **Neon rehearsal + prod migrate BATCHED**. Then P2-06 rate-limiter → P2-07 analytics-sink → P2-09 logger → P2-08 Payload (ISOLATED). **Loop discipline: ONE code packet per wave; workflow scripts PLAIN JS (no TS/nested-backticks/angle-brackets).**
