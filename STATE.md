...
- **Next concrete action**: P2-00/01/02/04/10/11 DONE (P2-04 7307d4e — repaired: calculateOrderTotals now the single charge source, both routes wired, flag-OFF mutation-proven byte-identical; P2-04a client-server-totals deferred). P2-05, P2-06, P2-07, P2-09 DONE (P2-09 0375815 — lib/log.ts). Next (LAST P2 code packet): **P2-08 Payload excision** (ISOLATED wave — mutates/removes deps): remove 5 @payloadcms/* + payload + payload.config.ts + payload:* scripts; archive scripts/migrate-payload-to-drizzle.ts → scripts/archive/; drop --legacy-peer-deps from CI; delete unused lenis. SCOUT all payload/lenis imports FIRST (removing a still-imported package breaks the build). Verify: `npm ci` WITHOUT the flag succeeds + `npm run verify` green; record bundle/install delta. Then **#G-P2 evidence pack** (batched, no auto-pass): schema-form demo, inventory-v2 rehearsal plan, GST flag, CI-green-without-legacy-peer-deps. Then P3∥P4 (P2-02a schema-driven SchemaForm prereq before P3-05/P4-02), P5, P6. **Loop discipline: ONE code packet per wave; workflow scripts PLAIN JS; EXCLUDE .env.production.example + untracked labs/ from commits.**
...
- **Packet specs can name non-load-bearing functions** (found at P2-04): the spec targeted `calculateOrderTotals` but it had ZERO prod callers — the real order-charge math is computed INLINE and DUPLICATED in `api/hono/routes/payments.ts` (create-order) + `api/hono/routes/orders.ts` (`taxAmountPaise = Math.round(subtotalPaise * GST_RATE)`, added on top, persisted). Always grep call sites before flag-gating/refactoring a "central" function. Adversarial review (L5) caught this — verifier alone passed it. Consolidating the two inline copies into calculateOrderTotals is the P2-04 repair.
