...
- **Next concrete action**: P2-00/01/02/04/10/11 DONE (P2-04 7307d4e — repaired: calculateOrderTotals now the single charge source, both routes wired, flag-OFF mutation-proven byte-identical; P2-04a client-server-totals deferred). P2-05 DONE (7ea2101 — inventory v2 schema + dual-write, flagged, migration built not run). Next: **P2-06 durable rate limiting** — lib/ports/rate-limiter.ts + in-memory adapter (default) + Upstash adapter (env-selected); wire payment:create/try-on/sign-up; adds @upstash deps (dep mutation → isolated wave); Upstash creds BATCHED. Then P2-07 analytics-sink → P2-09 logger → P2-08 Payload (ISOLATED). **Loop discipline: ONE code packet per wave; workflow scripts PLAIN JS (no TS/nested-backticks/angle-brackets).**
