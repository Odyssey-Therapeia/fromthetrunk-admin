...
export function ProductStepper({
  ...
  // Tracks the last `initialMedia` we synced, so we can re-sync during render
  // (instead of in an effect) when the prop changes.
  ...
  const lastPersistedSnapshotRef = useRef(
    serializeStepperValues(mergedInitialValues),
  );
  ...
  // Re-sync local media + the form's imageMediaIds when the incoming media set
  // changes (e.g. navigating between products). The local-state half is adjusted
  // during render instead of inside an effect, which avoids the cascading-render
  // warning; the form-store half stays in an effect (writing to an external store
  // is a legitimate effect use, and it isn't what the warning flags).
  ...
  useEffect(() => {
    const id = window.setInterval(() => {
      if (
        !hasStepperChanges(form.state.values, lastPersistedSnapshotRef.current)
      ) {
        return;
      }
  ...
  useEffect(() => {
    // TEMP debugging: if "armed" logs continuously while you are NOT typing,
    // form.state.values is handing back a new reference every render and this
    // debounce is looping (which would defeat the LivePreviewCard memo).
  ...
  // TEMP debugging: logs every ProductStepper render and flags which of these
  // refs changed since the last render.
  ...
  return (
    <div className="@container grid gap-6 @5xl:grid-cols-[1fr_320px]">
      <div className="space-y-4">
        ...
        {/* TEMP debugging: times the active step's render */}
        <Profiler id="step-area" onRender={onRenderProfiler}>
          <div ref={stepContainerRef}>
            ...
            {stepIndex === 6 ? (
              <StepPreview values={form.state.values} />
            ) : null}
          </div>
        </Profiler>
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="text-xs text-muted-foreground">
            {isSaving
              ? "Saving..."
              : (saveState ?? "Changes auto-save after edits")}
          </div>
      ...
      {/* TEMP debugging: times the live-preview render */}
      <Profiler id="live-preview" onRender={onRenderProfiler}>
        <LivePreviewCard imageUrls={previewImageUrls} values={previewValues} />
      </Profiler>
    </div>
  );
}
