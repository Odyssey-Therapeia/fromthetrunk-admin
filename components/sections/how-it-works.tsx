    ...
    <section className="mx-auto w-full max-w-6xl px-6">
      ...
      <BentoGrid className="mt-10 auto-rows-[16rem] grid-cols-1 gap-5 md:grid-cols-3 md:auto-rows-[18rem]">
        {steps.map((step, index) => {
          ...
          const image =
            featuredSarees[index]?.images[0] ?? featuredSarees[0]?.images[0];

          return (
            <ScrollReveal key={step.title} delay={index * 0.1}>
              <div className="group relative h-full overflow-hidden rounded-2xl border border-border/60 bg-card/70 shadow-soft">
                {image && (
                  <Image
                    src={image}
